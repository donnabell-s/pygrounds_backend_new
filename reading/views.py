...
from django.db.models import (
    IntegerField, Value, Count, Q, CharField
)
...
from .serializers import (
    ...
    NeighborIdsSerializer,   # (kept for compatibility, even if not used here)
    IdOnlySerializer,        # (kept for compatibility)
    TopicSerializer,         # (kept for compatibility)
    SubtopicSerializer,      # (kept for compatibility)
)
...
class TopicViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        topics = (
            CITopic.objects
            .annotate(subtopics_count=Count("subtopics"))  # use "subtopic_set" if CI.Subtopic.topic has no related_name
            .order_by("name")
...
class TopicListView(APIView):
    ...
    def get(self, request):
        topics = (
            CITopic.objects
            .annotate(subtopics_count=Count("subtopics"))  # use "subtopic_set" if needed
            .order_by("name")
            .values("id", "name", "slug", "subtopics_count")
        )
        return Response(list(topics))
...
class SubtopicListByTopicView(APIView):
    ...
    def get(self, request, topic_key):
        qs = (
            ...
            .annotate(materials_count=Count("reading_materials"))  # matches ReadingMaterial.subtopic_ref related_name
...
class TopicTOC(APIView):
    permission_classes = [AllowAny]
    def get(self, request, topic_key):
        topic = CITopic.objects.filter(Q(slug=topic_key) | Q(name=topic_key)).first()
        if not topic:
        ...
        qs = (
            ...
            .annotate(
                ...
                subtopic=Coalesce('subtopic_ref__name', Value('', output_field=CharField())),
            )
            .order_by('subtopic_ref__order_in_topic', '_ord', 'title', 'id')
            .values('id', 'title', 'subtopic')
        )
        return Response(list(qs))
